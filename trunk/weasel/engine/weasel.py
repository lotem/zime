...
class WeaselSessionHandler(SocketServer.StreamRequestHandler):
...
class WeaselService:
    ...
    MUTEX_NAME = "WeaselService"
    EVENT_NAME = "WeaselServiceReadyEvent"

    def run(self):
        if not self.check_single_instance():
            return
        self.service_ready_notify()
        addr = (WeaselService.HOST, WeaselService.PORT)
        self.server = SocketServer.TCPServer(addr, WeaselSessionHandler)
        logger.info("WeaselService running...")
        self.server.serve_forever()

    def check_single_instance(self):
        mutex = windll.kernel32.CreateMutexA(None, True, WeaselService.MUTEX_NAME)
        if not mutex:
            logger.error("error creating mutex '%s'; exiting." % WeaselService.MUTEX_NAME)
            return False
        if windll.kernel32.GetLastError() == ERROR_ALREADY_EXISTS:
            logger.info("existing WeaselService found; exiting.")
            windll.kernel32.CloseHandle(mutex)
            return False
        return True

    def service_ready_notify(self):
        event = windll.kernel32.OpenEventA(EVENT_ALL_ACCESS, False, WeaselService.EVENT_NAME)
        if not event:
            logger.warning("error opening event '%s'." % WeaselService.EVENT_NAME)
            return
